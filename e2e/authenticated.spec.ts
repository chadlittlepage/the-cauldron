...
test.describe('Authenticated flows', () => {
  ...
  test('can access submit track page', async ({ page }) => {
    ...
    await expect(page.getByText('Submit a Track')).toBeVisible();
    await expect(page.getByPlaceholder(/spotify|soundcloud/i)).toBeVisible();
  });
  ...
  test('submit form validates track URL', async ({ page }) => {
    ...
    const urlInput = page.getByPlaceholder(/spotify|soundcloud/i);
    await urlInput.fill('https://example.com/not-a-track');
    await urlInput.blur();
    // Should show a validation error or not enable the next step
    await expect(page.getByText(/supported platform|invalid|not recognized/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test('can vote on a track from browse', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      await expect(page.getByText(/session expired|sign in/i)).not.toBeVisible({ timeout: 3000 });
    }
  });
  ...
  test('can view track detail page', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      // Track detail should show title and embedded player or track info
  ...
  test('dashboard tabs are navigable', async ({ page }) => {
    ...
    await expect(page.getByText('Voting History')).toBeVisible();
